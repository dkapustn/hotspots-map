...
13. Запусти одиннадцатый файл — [`supabase/migrations/0011_guests_readonly.sql`](./supabase/migrations/0011_guests_readonly.sql) — запрещает гостям (анонимный вход) создавать метки, лайкать, посещать, комментировать, оценивать и сохранять (только просмотр). **Без неё гости смогут всё это делать в обход интерфейса.**
14. (Опционально) Включи гостевой вход: **Authentication → Sign In / Up → Anonymous Sign-Ins** → переключи в **Allow**. После этого на странице входа появится рабочая кнопка «Войти как гость».
